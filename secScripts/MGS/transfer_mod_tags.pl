...
sub donor_tags {
	...
	my ($mm, $ml, $mn);
	my $legacy_tag_names_normalized = 0;
	for my $field (@{$record}[11 .. $#{$record}]) {
		if ($field =~ /\AM[Mm]:/) {
			die "Donor '$name' has duplicate MM tags\n" if defined $mm;
			$mm = $field;
			$legacy_tag_names_normalized++ if $field =~ /\AMm:/;
		} elsif ($field =~ /\AM[Ll]:/) {
			die "Donor '$name' has duplicate ML tags\n" if defined $ml;
			$ml = $field;
			$legacy_tag_names_normalized++ if $field =~ /\AMl:/;
		} elsif ($field =~ /\AMN:/) {
	...
	my $implicit_mode_groups_normalized = 0;
	my @normalized_groups;
	my %position_probability;
	if ($groups ne '') {
		die "Donor '$name' has an unterminated MM group\n" unless $groups =~ /;\z/;
		for my $group (split /;/, $groups) {
			my ($base, $strand, $codes, $mode, $deltas) = $group =~ /\A([ACGTUN])([+-])([A-Za-z]+|\d+)([.?])?(?:,(\d+(?:,\d+)*))?\z/;
			...
			if (!defined $mode) {
				$mode = '.';
				$implicit_mode_groups_normalized++;
			}
			push @normalized_groups, "$base$strand$codes$mode"
				. (defined($deltas) ? ",$deltas" : '');
		}
	}
	...
	# Mm/Ml were draft tag names; newly published records use the standard
	# MM/ML names.  An absent mode has the same SAM semantics as '.', so making
	# it explicit preserves meaning and avoids modkit's legacy-mode gate.
	$mm = 'MM:Z:' . (@normalized_groups ? join(';', @normalized_groups) . ';' : '');
	$ml =~ s/\AMl:/ML:/;
...
my %counts = (
	...
	legacy_missing_mn_reads => 0, legacy_tag_names_normalized => 0,
	implicit_mode_groups_normalized => 0,
);
...
my $transfer_group = sub {
	...
	my ($mm, $ml, $missing_mn, $legacy_names, $implicit_modes) =
		donor_tags($donor_record, $donor_forward);
	$counts{legacy_missing_mn_reads} += $missing_mn;
	$counts{legacy_tag_names_normalized} += $legacy_names;
	$counts{implicit_mode_groups_normalized} += $implicit_modes;
...
# The donor is normally a superset of accepted representative alignments.  It
# must be consumed to EOF before closing the samtools pipe; otherwise a large
# trailing donor can make samtools receive SIGPIPE while still writing.  Using
# the normal class reader also validates natural-name ordering through the tail.
while (defined $donor_first) {
	my (undef, $next_donor) = take_natural_class($donor_first, $donor_fh, 'donor');
	$donor_first = $next_donor;
}
